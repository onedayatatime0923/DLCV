...
for j in ['zebra','mountain']:
    ...
    image=np.array([color[dm.data['kmeans'].labels_[i]] for i in range(len(dm.data['kmeans'].labels_))]).reshape((dm.data['image'].shape[0],dm.data['image'].shape[1],-1))
    dm.plot(image,'pic/{}_texture.png'.format(j))

    dm.read_image('image_rgb','{}.jpg'.format(j),mode='rgb')
    image=dm.data['image_rgb'].reshape(-1,3)
    feature=np.concatenate((feature,image),1)
    dm.kmeans('kmeans',feature,n=6)
    image=np.array([color[dm.data['kmeans'].labels_[i]] for i in range(len(dm.data['kmeans'].labels_))]).reshape((dm.data['image'].shape[0],dm.data['image'].shape[1],-1))
    dm.plot(image,'pic/{}_texture_rgb.png'.format(j))
'''
###############################################################
#                    problem 2.2                              #
###############################################################

dm.read_image('image','{}.jpg'.format('zebra'),mode='rgb')
dm.surf('surf',dm.data['image'])
dm.surf_plot('surf',dm.data['image'])
